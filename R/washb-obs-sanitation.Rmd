...
author: ""
date: "13 Nov 2017 (updated 27 Mar 2018)"
output: 
  html_notebook:
    theme: default
    highlight: haddock
...
To download the R markdown source code for this notbook, toggle the dropdown menu at the top right of this page to "Download Rmd".

## Preamble
```{r preamble, eval=TRUE,echo=TRUE,results='hide'}
# Preamble - load packages and save colors for plotting

library(washb) # development package available at devtools::install_github("ben-arnold/washb")
...
```{r bangladesh density, echo=FALSE}
...
dk_enrol <- read.csv("~/washb-obs-san/data/table1.csv")
dk_anth  <- read.csv("~/washb-obs-san/data/endline-anthro.csv")

# restrict enrollment measures to improved latrine and merge
# to the anthropometry measures at the year 2 visit
dk_enrol <- dk_enrol %>%
  select(clusterid,compoundid,hhid,implat)
dk <- left_join(dk_anth,dk_enrol,by=c("clusterid","compoundid","hhid"))
...
kWs <- c("month","HHS","aged","sex","mother_age","motherht","mother_edu",
